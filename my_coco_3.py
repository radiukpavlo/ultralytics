...
def main(num_images, batch_size, num_workers):
    """
    ...
    train_loader = DataLoader(coco_train, batch_size=batch_size, shuffle=False, num_workers=num_workers, pin_memory=True)
    ...
    for i, (images, targets) in enumerate(train_loader):
        try:
            ...
            for j in range(len(images)):
                ...
                if not targets[j]:
                    print(f"Skipping image {i * batch_size + j} due to empty targets")
                    continue

                # Process the image
                processed_image = process_image(images[j], device)

                # Extract the image ID from targets or filename
                if targets[j]:
                    image_id = targets[j][0]['image_id'].item()  # Convert tensor to int
                else:
                    image_id = extract_image_id_from_filename(coco_train.dataset.coco.loadImgs(i * batch_size + j)[0]['file_name'])
                ...
                if (i * batch_size + j) % 100 == 0:
                    print(f'Processed {i * batch_size + j} images')
        except FileNotFoundError as e:
            print(f"File not found: {e}")
        except IndexError as e:
            print(f"Skipping image {i * batch_size + j} due to IndexError: {e}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--batch_size', type=int, default=1, help='The number of images to process per batch.')
    parser.add_argument('--num_workers', type=int, default=4, help='The number of worker processes to use for data loading.')
    args = parser.parse_args()

    main(args.num_images, args.batch_size, args.num_workers)
